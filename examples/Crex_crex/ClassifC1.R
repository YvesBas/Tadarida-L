...
#
#   is called and is needed by Ta_Tc
#   
#############################################################
...
#obslist=list.files(tafolder[a],pattern=".ta$",full.names=T,recursive=T) # 1e-4 sec/files

skip=as.logical(args[16])
...
if (mean(testReduc)>0.5) #when a reduction of variables is necessary (=low frequency case)
{
  ...
  Reduc2=predict(Scaling,CTP[,5:274])
...
# do we really need to write this very big file? Could speed up things not to do so. DD4WH 12.9.2019
