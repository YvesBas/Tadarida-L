...
#   write_tabase3HF.R
#
#   Prepare a tabase3HF_x.csv file which is needed to build a Classifier
#   
# 
#   Input:    VarSel.csv   SpeciesList.csv   
#             RSDB with  .eti files    .ta files 
#
#   Output:   tabase3HF_x.csv
#
#################################################################################
...
#required:
RSDB="e:/Montpellier/Crex/RDBlow"
...
#optional: uncomment, if you are only interested in ultrasound (above 8kHz)
#LowFrequencyHighpass = 8
...
etidir="e:/Montpellier/Crex/RDBlow/eti" #path where labels (.eti files) are stored 
pardir="e:/Montpellier/Crex/RDBlow/txt" #path where features (.ta files) are stored 
...
fichier2=vector()
for (i in 1:length(etilist1)){ # 5 minutes
...
Sys.time()
etitot=as.data.frame(rbindlist(my.data,fill=T,use.names=T))
...
Sys.time()
my.data <- list()
for (i in 1:length(parlist1)){ # 10 min
#for (i in 1:20300){
...
param3=as.data.frame(rbindlist(my.data,fill=T,use.names=T))

# optional frequency filtering
if (exists("LowFrequencyHighpass")==T) param3=subset(param3,param3$FreqMP>=LowFrequencyHighpass)

param4=subset(param3,select=c("Filename","CallNum",VarSel$V1))
...
DSESpecies = as.list(table(tabase3$Espece))
fwrite(DSESpecies, "DSE_species.csv")


#converting (rarely) missing features to 0 #A SUPPRIMER
#tabase3[is.na(tabase3)]=0

# this finally writes the desired table that is then being used to build the classifier
